...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        #     )
            if args.model in ['vit_small', 'vit_base']:
            ...
            if not args.cosub:
                ...
                elif args.data_set == 'STREET':
                    logits_bin4 = outputs[:, 9:11]  # Binary task: streetlight
                    loss = criterion(samples, logits_bin4, targets)
                else:
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        with amp_autocast():
            ...
            if args.data_set == 'COMBINED':
                ...
                logits_bin4 = output[:, 9:11]  # Binary: streetlight
                loss = criterion(logits_bin4, target)
                preds = logits_bin4.argmax(dim=1)
        ...
        if args.data_set != 'COMBINED':
            if args.data_set == 'STREET':
                acc1 = accuracy(logits_bin4, target, topk=(1,))[0]
            else:
