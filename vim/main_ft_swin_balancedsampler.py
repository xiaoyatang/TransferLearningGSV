...
def get_args_parser():
    ...
    parser.add_argument('--batch-size', default=2, type=int) # default: 64
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data_set', default='STREET', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'STREET','GREEN30','SIDEWALKS','SPEEDBUMPS'],
...
def load_checkpoint_flexibly(model, ckpt_path):
    first_param = next(model.parameters())
    print(f"Before loading--Mean of first loaded weight: {first_param.data.mean().item():.5f}")
    second_param = next(model.parameters())
    print(f"Before loading-- Mean of second loaded weight: {second_param.data.mean().item():.5f}")
    ...
    try:
        first_param = next(model.parameters())
        print(f"After loading-- Mean of first loaded weight: {first_param.data.mean().item():.5f}")
        second_param = next(model.parameters())
        print(f"After loading-- Mean of second loaded weight: {second_param.data.mean().item():.5f}")
    except:
        pass
...
def main(args):
    ...
    data_loader_val = torch.utils.data.DataLoader(
        ...
        batch_size=args.batch_size, # by Xiaoya
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=False
    )
    ...
    if args.model == 'swin_t':
        model = torchvision_models.swin_t(weights=torchvision_models.Swin_T_Weights.IMAGENET1K_V1)
    elif args.model == 'swin_s':
        model = torchvision_models.swin_s(weights=torchvision_models.Swin_S_Weights.IMAGENET1K_V1)
    elif args.model == 'swin_b':
        model = torchvision_models.swin_b(weights=torchvision_models.Swin_B_Weights.IMAGENET1K_V1)
    elif args.model == 'swin_b384':
        from transformers import AutoFeatureExtractor, SwinForImageClassification
        # AutoFeatureExtractor.from_pretrained("microsoft/swin-base-patch4-window12-384-in22k")
        model = SwinForImageClassification.from_pretrained("microsoft/swin-base-patch4-window12-384-in22k")
    else:
        raise ValueError(f"Unsupported model_name: {args.model}")

    if args.model == 'swin_b384':
        in_features = model.classifier.in_features
        model.classifier = torch.nn.Linear(in_features, args.nb_classes)
        # print(model)
    else:
        in_features = model.head.in_features
        model.head = torch.nn.Linear(in_features, args.nb_classes)
    # for i, stage in enumerate(model.features):   #for table 1 in gsv paper, specially for vit/swin
    #     print(f"Stage {i}: {stage.blocks[0].attn.num_heads}")
    # print("Inspecting Swin stages...")
    # for i, stage in enumerate(model.features):
    #     blk = None
    #     if isinstance(stage, torch.nn.Sequential):
    #         for m in stage:
    #             if hasattr(m, "attn"):   # Swin block
    #                 blk = m
    #                 break
    #     if blk is not None:
    #         print(f"Stage {i}: num_heads = {blk.attn.num_heads}")
    #     else:
    #         print(f"Stage {i}: (no attention block)")

    # ======== Before finetune checkpoint load ========
    print("Checking model parameters before finetune loading...")
    found_param = False
    for name, param in model.named_parameters():
        if any(x in name for x in ['layers', 'stages']):
    ...
    if args.finetune:
        ...
        # ----- handle timm/ImageNet checkpoint -----
...
# ======== Optional attention-only fine-tune ========
    if args.attn_only:
        print("⚙️ Setting attention-only mode (mainly for Swin).")
...
# ======== Model to device & wrappers ========
    ...
    if args.resume: #resume from pretrained ckp,这里稍微改了一些判断逻辑避免load ckp报错
        checkpoint = load_checkpoint_flexibly(model_without_ddp, args.resume)
        ...
        # if hasattr(model_without_ddp, 'blocks'):
        #     print(f"After loading pretrained weights, blocks.0.norm1.weight: "
        #         f"{model_without_ddp.blocks[0].norm1.weight.data.mean().item():.4f}")
        # elif hasattr(model_without_ddp, 'layer1'):
        #     print(f"After loading pretrained weights (ResNet style), layer1.0.conv1.weight mean: "
        #         f"{model_without_ddp.layer1[0].conv1.weight.data.mean().item():.4f}")
        # else:
        #     print("After loading pretrained weights: no recognizable block/layer to check.")
        # if missing_keys:
